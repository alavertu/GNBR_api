...
for themeFile, depPathFile, outFile in zip(themeFiles, depPathFiles , outFiles):
    ...
    with open(outFile, "w") as outCsv:
        ...
        for key in netOut:
            ...
            distro = list( map(int, netOut.get(key).tolist()) )
            codes = zip( distro[::2], depDict["header"][1::2], [i > 0 for i in distro[1::2]] )
            max_codes = max(codes, key = lambda x: x[0])
            info = info + list(map(int, netOut.get(key).tolist())) + list(max_codes)
